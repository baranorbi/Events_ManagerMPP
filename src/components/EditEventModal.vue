<template>
  <div class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4 overflow-y-auto">
    <div class="bg-[#232323] rounded-lg w-full max-w-4xl mx-auto border border-[#737373]">
      ...
      <div v-else-if="error" class="p-6 flex flex-col items-center justify-center min-h-[300px]">
...
onMounted(async () => {
  ...
  try {
    const event = await eventStore.getEventById(props.eventId);
    if (event) {
...
const validateAndSaveEvent = async () => {
  ...
  try {
    ...
    console.log('Saving updated event:', editedEvent.value);
    
    // Call the async updateEvent function
    const result = await eventStore.updateEvent(editedEvent.value);
    ...
    if (error instanceof Error) {
      validationErrors.value.push(`Update failed: ${error.message}`);
    } else {
      validationErrors.value.push('Failed to update event. Please try again.');
    }
  } finally {
    isSubmitting.value = false;
  }
