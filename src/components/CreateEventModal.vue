<template>
  <div class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4 overflow-y-auto">
    <div class="bg-[#232323] rounded-lg w-full max-w-4xl mx-auto border border-[#737373]">
      <div class="p-6">
        ...
        <div class="flex flex-col lg:flex-row gap-6">
          ...
          <div class="flex-1">
            ...
            <div class="mb-6">
              <div class="text-sm text-[#737373] mb-2">Event Image</div>
            ...
            <div class="mb-4">
              <div class="text-sm text-[#737373] mb-1">Select date</div>
              <div class="relative">
                <input 
                  v-model="dateString"
                  type="text" 
            ...
            <div class="mb-4">
              <div class="text-sm text-[#737373] mb-1">Time</div>
              <div class="grid grid-cols-2 gap-4">
                <div class="relative">
                  ...
                  />
                  <Clock class="absolute right-3 top-1/2 transform -translate-y-1/2 text-[#737373]" :size="20" />
                </div>
                <div class="relative">
                  <input 
            ...
            <div class="mb-4">
              <div class="text-sm text-[#737373] mb-1">Location</div>
              <div class="relative">
                <input 
            ...
            <div class="mb-4">
              <div class="text-sm text-[#737373] mb-1">Category</div>
              <div class="relative">
                <select
            ...
            <div class="mb-4">
              <div class="flex items-center">
                ...
                />
                <label for="isOnline" class="ml-2 text-sm text-[#737373]">This is an online event</label>
              </div>
            </div>
          </div>
          ...
          <div class="lg:w-1/3 flex flex-col justify-center">
            <!-- Card Preview -->
            <div v-if="event.title && previewUrl">
              ...
              <div class="bg-[#232323] rounded-lg overflow-hidden border border-[#737373] shadow">
                ...
                <div class="p-3">
                  <h3 class="font-medium text-[#D9D9D9] truncate">{{ event.title }}</h3>
      ...
      <div class="flex border-t border-[#737373]">
        <button 
          @click="$emit('close')"
          class="flex-1 py-4 text-center text-[#D9D9D9] hover:bg-[#333333] transition-colors"
        >
          Cancel
        </button>
        <button 
          @click="validateAndCreateEvent"
          class="flex-1 py-4 text-center bg-[#533673] text-white hover:bg-opacity-90 transition-colors rounded-br-lg"
...
const isSelectedDate = (date: Date) => {
  return date.toDateString() === event.value.date.toDateString();
};
...
const hideDatePicker = () => {
  // to allow click events to register before hiding
  setTimeout(() => {
    showDatePicker.value = false;
  }, 200);
};
...
const updatePreviewUrl = () => {
  if (imageUrl.value && 
    ...
    invalidUrl.value = true;
    // don't update preview if URL is invalid
  } else {
    invalidUrl.value = false;
    previewUrl.value = '';
  }
...
const isValidTimeFormat = (time: string): boolean => {
  ...
  // Aformats like "1:00 PM", "13:00", "1:00pm", "1:00 pm"
...
const validateForm = (): boolean => {
  ...
  } else if (event.value.title.length < 3) {
  ...
  } else if (event.value.description.length < 10) {
  ...
  if (event.value.startTime && !isValidTimeFormat(event.value.startTime)) {
  ...
  if (event.value.endTime && !isValidTimeFormat(event.value.endTime)) {
