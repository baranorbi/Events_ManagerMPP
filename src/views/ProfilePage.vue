<template>
    <AppLayout>
      <div class="max-w-6xl mx-auto">
        ...
        <div v-if="interestedEvents.length > 0" class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <EventCard 
  ...
  onMounted(() => {
    userData.value = eventStore.getUserData();
    loadInterestedEvents();
  });
  
  const loadInterestedEvents = () => {
    interestedEvents.value = eventStore.getInterestedEvents();
  };
  
  const removeFromInterested = (eventId: string) => {
    if (eventStore.removeFromInterested(eventId)) {
      loadInterestedEvents();
    }
  };
